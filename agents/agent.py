...
import json
import requests
import google.generativeai as genai # Needed for check_gemini_models
import time # Still useful for general purpose delays if ever needed
# from datetime import datetime # No longer needed without web scraping
...
from langchain_core.prompts import ChatPromptTemplate, MessagesPlaceholder # CRITICAL: Ensure MessagesPlaceholder is imported
from langchain.chains import create_history_aware_retriever, create_retrieval_chain
from langchain.chains.combine_documents import create_stuff_documents_chain
from langchain_core.messages import HumanMessage, AIMessage
from langchain_core.documents import Document

# Imports for LangChain Agents and Tools (Keep `Tool` for RAG tool, but others removed)
...
agent_executors = {}
db = None
all_agent_tools = [] # List to hold only the document_qa_retriever tool now
...
def check_gemini_models():
    ...
    try:
        ...
        if configured_model in available_model_names:
            ...
            logger.info("--- End of available models check ---")
...
def initialize_rag_components(model_mode: str):
    ...
    """
    global llm, retriever, db, all_agent_tools # Removed tool_manager_instance

    # Ensure necessary directories exist.
    for directory in [config.DOCUMENT_STORAGE_DIRECTORY, config.CHROMA_DB_DIRECTORY]:
        try:
    ...
    # --- 1. Load Documents from Local files only ---
    ...
    logger.info(f"Total documents for RAG: {len(all_documents)}")
    ...
    if model_mode == "local":
        ...
        if not api_key:
            ...
            sys.exit(1) # Exit if no API key for cloud mode
        genai.configure(api_key=api_key)
    ...
    # Only the document_qa_retriever tool will be added now.
...
def api_bot():
    ...
    if session_id not in agent_executors:
        ...
        history_aware_retriever_prompt = ChatPromptTemplate.from_messages([
            ...
            MessagesPlaceholder(variable_name="chat_history"), # Correct placeholder for history
            ("user", "{input}"),
        ...
        # 3. Define the tools the agent can use (only RAG tool now)
        session_tools = []
        
        session_tools.append(Tool(
            name="document_qa_retriever",
            func=_run_document_qa_retriever_tool_for_session,
        ...
        logger.info("Added 'document_qa_retriever' tool for the session.")

        logger.info(f"Total tools passed to agent for session {session_id}: {len(session_tools)}")
        # DEBUGGING TOOL LIST (removed as only one tool now, but keep for future use if more added)
        ...
        agent_prompt = ChatPromptTemplate.from_messages([
            ("system", """You are a diligent and accurate Document Assistant. Your primary goal is to answer questions by looking through the provided documents.
            
            **Tool Usage Guidelines:**
            - **Document Retrieval:** Use `document_qa_retriever` to answer questions by looking through your uploaded documents. This is your primary source of knowledge.
            
            **General Knowledge Fallback:**
            - If a question **cannot be answered by your document tool**, you may use your inherent general knowledge to provide a concise and relevant answer directly. Do not apologize for not using a tool if you can answer directly.
            
            **Response Style:**
            - Always provide a direct and factual answer.
            - Think step-by-step in your `agent_scratchpad` to determine the best approach (whether to use the document tool or general knowledge)."""),
            MessagesPlaceholder(variable_name="chat_history"),
            ("human", "{input}"),
            MessagesPlaceholder(variable_name="agent_scratchpad"),
        ])

        agent = create_tool_calling_agent(llm, session_tools, agent_prompt) # Use session_tools
    ...
    try:
        ...
        if isinstance(e, google_exceptions.InternalServerError) or (
            ...
            user_message = (
                ...
                "This typically happens when the conversation history is very long. "
